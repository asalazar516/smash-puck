	...
	if texture == player1Text:
		...
		if(Input.is_action_just_pressed("player_2_right")):
			...
			# If the cursor goes past the total amount of columns and amount of characters, reset to the first item in the whole roster 
				...
				position.x += portraitOffset.x
		elif(Input.is_action_just_pressed("player_2_left")):
			currentSelected -= 1
			currentColumnSpot -= 1
		...
		if(Input.is_action_just_pressed("player_2_slap_right")):
			if(CharacterSelectionManager.opponent == null):
				CharacterSelectionManager.opponent = CharacterSelectionManager.selectable_characters[characters[currentSelected].name + " Back"]
				print(CharacterSelectionManager.opponent)
				get_tree().change_scene("res://Scenes/levels/comp_level_selection.tscn")
